...
-->
	...
	<body>
		<table border="0" align="center" cellspacing="1" bgcolor="#336699"
			...
			%>
			<c:forEach var="join"
				items="${mine:getMeetingJoinByManagerID(server.mmanagerID,showPage,ipage)}"
				varStatus="num">
				<tr>
					<td bgcolor="#FFFFFF">
					...
					<td bgcolor="#FFFFFF">
						<a href="server/oneuser.jsp?userID=${join.userID}">查看信息</a>
					</td>
					<td bgcolor="#FFFFFF">
						<a href="server/setserver.jsp?userID=${join.userID}">查看出行计划</a>
					</td>
				</tr>
			</c:forEach>
			<tr>
				<td colspan="6" bgcolor="#FFFFFF">
					<div align="right">
						<form action="server/allusers.jsp">
							共
							<font color=red>${rowCount}</font>条&nbsp; ${ipage }条/页&nbsp; 第
							<font color=red>${showPage }</font>页
							<a href="server/allusers.jsp?showPage=1">[首页]</a>&nbsp;

							<c:choose>
								<c:when test="${showPage>1}">
									<a href="server/allusers.jsp?showPage=${showPage-1 }">[上一页]</a>&nbsp;
						</c:when>
								<c:otherwise>
							[上一页]&nbsp;
						</c:otherwise>
							...
							<c:choose>
								<c:when test="${showPage < pageCount}">
									<a href="server/allusers.jsp?showPage=${showPage+1 }">[下一页]</a>&nbsp;
						</c:when>
								<c:otherwise>
							[下一页]&nbsp;
						</c:otherwise>
							...
							<a href="server/allusers.jsp?showPage=${pageCount }">[尾页]</a>&nbsp;
							转到
						...
						<form action="server/allusers.jsp?submitFlag=1" method="post">
			...
			%>
			<c:forEach var="join"
				items="${mine:getMeetingJoinByManagerIDFromKey(server.mmanagerID,searchKey1)}"
				varStatus="num">
				<tr>
					<td bgcolor="#FFFFFF">
						${(showPage-1)*ipage+num.count}
