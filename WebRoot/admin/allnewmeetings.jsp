...
<c:set var="rowCount" scope="page"
	value="${mine:getMeetingCountBystatus('N')}"></c:set>
...
<body>
	<table border="0" align="center" cellspacing="1" bgcolor="#336699"
		width="98%">
		<tr>
			<th align="center" colspan="7" style="font-size: 16px;height: 30px;">待审核的会议</th>
		</tr>
		<tr>
		...
		<c:forEach var="meeting"
			items="${mine:getAllMeeting(showPage,ipage,'N')}" varStatus="num">
			<tr>
				...
				<td bgcolor="#FFFFFF"><a
					href="admin/onemeeting.jsp?meetingID=${meeting.meetingID}">查看</a>
				</td>
				<td bgcolor="#FFFFFF"><a
					href="admin/allpassmeetings.jsp?passID=${meeting.meetingID}&showPage=${showPage}"
					onclick="javascript:if(confirm('通过后用户可见且可参与，是否通过?')){return true}else{return false;}">通过</a>
				</td>
				<td bgcolor="#FFFFFF"><a
					href="admin/allnewmeetings.jsp?delID=${meeting.meetingID}&showPage=${showPage}"
		...
		<tr>
			<td colspan="7" bgcolor="#FFFFFF">
				<div align="right">
					<form action="admin/allnewmeetings.jsp">
						共 <font color=red>${rowCount}</font>条&nbsp; ${ipage }条/页&nbsp; 第 <font
							color=red>${showPage }</font>页 <a
							href="admin/allnewmeetings.jsp?showPage=1">[首页]</a>&nbsp;

						<c:choose>
							<c:when test="${showPage>1}">
								<a href="admin/allnewmeetings.jsp?showPage=${showPage-1 }">[上一页]</a>&nbsp;
						</c:when>
							<c:otherwise>
							[上一页]&nbsp;
						</c:otherwise>
						...
						<c:choose>
							<c:when test="${showPage < pageCount}">
								<a href="admin/allnewmeetings.jsp?showPage=${showPage+1 }">[下一页]</a>&nbsp;
						</c:when>
							<c:otherwise>
							[下一页]&nbsp;
						</c:otherwise>
						...
						<a href="admin/allnewmeetings.jsp?showPage=${pageCount }">[尾页]</a>&nbsp;
					...
					<form action="admin/allnewmeetings.jsp?submitFlag=1" method="post">
		...
		%>
		<c:forEach var="meeting"
			items="${mine:getAllMeetingFromKey('N',searchKey1)}" varStatus="num">
			<tr>
				<td bgcolor="#FFFFFF">${(showPage-1)*ipage+num.count}</td>
				<td bgcolor="#FFFFFF">${meeting.meetingname}</td>
				<td bgcolor="#FFFFFF">${meeting.managername}</td>
