...
<c:choose>
	<c:when test="${not empty param.blackID}">
		<c:choose>
			<c:when test="${mine:changeMeetingStatus(param.blackID,'O')}">
				<script>
					alert("会议拉黑成功");
				</script>
			</c:when>
...
<body>
	<table border="0" align="center" cellspacing="1" bgcolor="#336699"
		width="98%">
		<tr>
			<th align="center" colspan="7" style="font-size: 16px;height: 30px;">黑名单的会议</th>
		</tr>
		<tr>
		...
		<c:forEach var="meeting"
			items="${mine:getAllMeeting(showPage,ipage,'O')}" varStatus="num">
			<tr>
				...
				<td bgcolor="#FFFFFF"><a
					...
					onclick="javascript:if(confirm('恢复后用户可见，且会议管理员可登录管理，确定恢复?')){return true}else{return false;}">恢复</a>
				</td>
				<td bgcolor="#FFFFFF"><a
					href="admin/allblackmeetings.jsp?delID=${meeting.meetingID}&showPage=${showPage}"
		...
		<tr>
			<td colspan="7" bgcolor="#FFFFFF">
				<div align="right">
					<form action="admin/allblackmeetings.jsp">
						共 <font color=red>${rowCount}</font>条&nbsp; ${ipage }条/页&nbsp; 第 <font
							color=red>${showPage }</font>页 <a
							href="admin/allblackmeetings.jsp?showPage=1">[首页]</a>&nbsp;

						<c:choose>
							<c:when test="${showPage>1}">
								<a href="admin/allblackmeetings.jsp?showPage=${showPage-1 }">[上一页]</a>&nbsp;
						</c:when>
							<c:otherwise>
							[上一页]&nbsp;
						</c:otherwise>
						...
						<c:choose>
							<c:when test="${showPage < pageCount}">
								<a href="admin/allblackmeetings.jsp?showPage=${showPage+1 }">[下一页]</a>&nbsp;
						</c:when>
							<c:otherwise>
							[下一页]&nbsp;
						</c:otherwise>
						...
						<a href="admin/allblackmeetings.jsp?showPage=${pageCount }">[尾页]</a>&nbsp;
					...
					<form action="admin/allblackmeetings.jsp?submitFlag=1"
						method="post">
		...
		%>
		<c:forEach var="meeting"
			items="${mine:getAllMeetingFromKey('O', searchKey1)}" varStatus="num">
			<tr>
				<td bgcolor="#FFFFFF">${(showPage-1)*ipage+num.count}</td>
				<td bgcolor="#FFFFFF">${meeting.meetingname}</td>
				<td bgcolor="#FFFFFF">${meeting.managername}</td>
