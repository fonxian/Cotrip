...
<c:set var="rowCount" scope="page"
	value="${mine:getSysNNCountByType('P')}"></c:set>
...
<body>
	<table border="0" align="center" cellspacing="1" bgcolor="#336699"
		width="98%">
		<tr>
			<th align="center" colspan="4" style="font-size: 16px;height: 30px;">系统公告管理</th>
		</tr>
		<tr>
			<td bgcolor="#EDEDE1">公告编号</td>
			<td bgcolor="#EDEDE1">公告标题</td>
		...
		<c:forEach var="newsnotices"
			items="${mine:getAllNewsNotices(showPage,ipage,'P')}" varStatus="num">
			<tr>
				...
				<td bgcolor="#FFFFFF"><a
					href="admin/allnotices.jsp?delID=${newsnotices.sysnnID}&showPage=${showPage}"
		...
		<tr>
			<td colspan="5" bgcolor="#FFFFFF">
				<div align="right">
					<form action="admin/allnotices.jsp">
						共 <font color=red>${rowCount}</font>条&nbsp; ${ipage }条/页&nbsp; 第 <font
							color=red>${showPage }</font>页 <a
							href="admin/allnotices.jsp?showPage=1">[首页]</a>&nbsp;

						<c:choose>
							<c:when test="${showPage>1}">
								<a href="admin/allnotices.jsp?showPage=${showPage-1 }">[上一页]</a>&nbsp;
						</c:when>
							<c:otherwise>
							[上一页]&nbsp;
						</c:otherwise>
						...
						<c:choose>
							<c:when test="${showPage < pageCount}">
								<a href="admin/allnotices.jsp?showPage=${showPage+1 }">[下一页]</a>&nbsp;
						</c:when>
							<c:otherwise>
							[下一页]&nbsp;
						</c:otherwise>
						...
						<a href="admin/allnotices.jsp?showPage=${pageCount }">[尾页]</a>&nbsp;
					...
					<form action="admin/allnotices.jsp?submitFlag=1" method="post">
		...
		%>
		<c:forEach var="newsnotices"
			items="${mine:getAllNewsNoticesFromKey('P',searchKey1)}" varStatus="num">
			<tr>
				<td bgcolor="#FFFFFF">${(showPage-1)*ipage+num.count}</td>
				<td bgcolor="#FFFFFF">${newsnotices.sysnnname}</td>
				<td bgcolor="#FFFFFF">${newsnotices.sysnntime}</td>
