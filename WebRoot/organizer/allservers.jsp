...
<c:choose>
	<c:when test="${not empty param.delID}">
		<c:choose>
			<c:when test="${mine:delMeetingManagerByID(param.delID)}">
				<script>
					alert("删除成功");
				</script>
			</c:when>
...
<c:set var="rowCount" scope="page"
	value="${mine:getMeetingManagerCountByMeetingID(meeting.meetingID)}"></c:set>
	...
	<body>
		<table border="0" align="center" cellspacing="1" bgcolor="#336699"
			...
			<%
				...
				if (submitFlag == null) {
			%>
			<c:forEach var="server"
				items="${mine:getAllMeetingManager(showPage,ipage,meeting.meetingID)}"
				varStatus="num">
				<tr>
					<td bgcolor="#FFFFFF">
					...
					<td bgcolor="#FFFFFF">
						<a href="organizer/changeserver.jsp?mmID=${server.mmanagerID}">修改</a>
					</td>
					<td bgcolor="#FFFFFF">
						<c:if test="${not mine:haveManagerID(server.mmanagerID)}">
							<a href="organizer/allservers.jsp?delID=${server.mmanagerID}"
			...
			<tr>
				<td colspan="6" bgcolor="#FFFFFF">
					<div align="right">
						<form action="organizer/allservers.jsp">
							共
							<font color=red>${rowCount}</font>条&nbsp; ${ipage }条/页&nbsp; 第
							<font color=red>${showPage }</font>页
							<a href="organizer/allservers.jsp?showPage=1">[首页]</a>&nbsp;

							<c:choose>
								<c:when test="${showPage>1}">
									<a href="organizer/allservers.jsp?showPage=${showPage-1 }">[上一页]</a>&nbsp;
						</c:when>
								<c:otherwise>
							[上一页]&nbsp;
						</c:otherwise>
							...
							<c:choose>
								<c:when test="${showPage < pageCount}">
									<a href="organizer/allservers.jsp?showPage=${showPage+1 }">[下一页]</a>&nbsp;
						</c:when>
								<c:otherwise>
							[下一页]&nbsp;
						</c:otherwise>
							...
							<a href="organizer/allservers.jsp?showPage=${pageCount }">[尾页]</a>&nbsp;
							转到
						...
						<form action="organizer/allservers.jsp?submitFlag=1" method="post">
			...
			%>
			<c:forEach var="server"
				items="${mine:getAllMeetingManagerFromKey(meeting.meetingID,searchKey1)}"
				varStatus="num">
				<tr>
					<td bgcolor="#FFFFFF">
						${(showPage-1)*ipage+num.count}
