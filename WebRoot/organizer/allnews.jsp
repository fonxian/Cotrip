	...
	<body>
		<table border="0" align="center" cellspacing="1" bgcolor="#336699"
			...
			<c:forEach var="newsnotices"
				items="${mine:getMeetingNewsNoticesByType(showPage,ipage,meeting.meetingID,'N')}"
				varStatus="num">
				<tr>
					<td bgcolor="#FFFFFF">
					...
					<td bgcolor="#FFFFFF">
						<a
							href="organizer/allnews.jsp?delID=${newsnotices.mnnID}&showPage=${showPage}"
							oncl javascri pt: if (confirm('删除后无法恢复，是否删除?')) {
		returntrue
	} else {
		returnfalse;
	};
>删除</a>
			...
			<tr>
				<td colspan="5" bgcolor="#FFFFFF">
					<div align="right">
						<form action="organizer/allnews.jsp">
							共
							<font color=red>${rowCount}</font>条&nbsp; ${ipage }条/页&nbsp; 第
							<font color=red>${showPage }</font>页
							<a href="organizer/allnews.jsp?showPage=1">[首页]</a>&nbsp;

							<c:choose>
								<c:when test="${showPage>1}">
									<a href="organizer/allnews.jsp?showPage=${showPage-1 }">[上一页]</a>&nbsp;
						</c:when>
								<c:otherwise>
							[上一页]&nbsp;
						</c:otherwise>
							...
							<c:choose>
								<c:when test="${showPage < pageCount}">
									<a href="organizer/allnews.jsp?showPage=${showPage+1 }">[下一页]</a>&nbsp;
						</c:when>
								<c:otherwise>
							[下一页]&nbsp;
						</c:otherwise>
							...
							<a href="organizer/allnews.jsp?showPage=${pageCount }">[尾页]</a>&nbsp;
							转到
						...
						<form action="organizer/allnews.jsp?submitFlag=1" method="post">
			...
			%>
			<c:forEach var="newsnotices"
				items="${mine:getMeetingNewsNoticesByTypeFromKey(meeting.meetingID,'N',searchKey1)}"
				varStatus="num">
				<tr>
					<td bgcolor="#FFFFFF">
						${(showPage-1)*ipage+num.count}
					...
					<td bgcolor="#FFFFFF">
						<a
							href="organizer/allnews.jsp?delID=${newsnotices.mnnID}&showPage=${showPage}"
							ja vascript: if (confirm('删除后无法恢复，是否删除?')) {
		returntrue
	} else {
		returnfalse;
	};
