...
<html>
  ...
  <body>
  	<form action="servlet/SetServer" method="post">
    ...
    <tr><td bgcolor="#EDEDE1" colspan="8" height="40px">
    	设置接待人员
    	<select name="managerID">
    		<c:forEach var="server" items="${mine:getMeetingManager(meeting.meetingID)}">
    			<option value="${server.mmanagerID}">${server.name }</option>
    		</c:forEach>
    	</select>
    	<input type="checkbox" name="group" value="group"/>是否对整组与会者设置此接待员
    ...
    <c:choose>
    	<c:when test="${not mine:haveTravelPlan(meeting.meetingID,param.userID) and not mine:haveResponse(meeting.meetingID,param.userID)}">
    	<tr><td colspan="8" align="center" bgcolor="#FFFFFF">没有发布或响应出行计划</td></tr>
    	</c:when>
    	
    	<c:when test="${mine:haveTravelPlan(meeting.meetingID,param.userID)}">
    	<c:set var="plan" value="${mine:getTravelPlanByuserIDAndMeetingID(meeting.meetingID,param.userID)}"></c:set>
    	<tr><td   <c:choose>
    	...
    	<c:when test="${mine:haveResponse(meeting.meetingID,param.userID)}">
    		<c:set var="ID" value="${mine:getPlanID(meeting.meetingID,param.userID)}"></c:set>
